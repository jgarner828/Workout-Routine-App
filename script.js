...
let name = '';   // user name 
let programLength = 8;  // how many weeks the program will run


// need an exercise object.... 
let exercise = {
...
2A. Create a user. Create a program. Start with how many weeks you want to run the program. Show days of the week with name of each exercise. Allow user to add a new exercise object with all training data necessary. Allow list items to get moved in priority ranking. When user is satisfied, allow user to create program.

2B. Load program based on day. (Possibly use moment.js?) The program checks which objects to call based on frequency (boolean value) and call them into an array. App runs through each exercise in array allowing the user to log their actual results. If total reps is less than reps*sets value then decrease by increment and save. If total reps is more than sets*reps then increase by increment and save.
